...
public class TestEmployee extends TestCase {
    ...
    public void testConstructor() {
        ...
        TestCase.assertEquals("employee name wrong", employee_name, emp.getEmpName());
        TestCase.assertTrue("employee no. wrong", emp.getEmpNumber().equals(employee_no));

        // test invalid inputs - null emp no and null emp name
        try {
            Employee e = new Employee("Peter Brown", null);
            TestCase.fail("Constructor allows null employee number");
        ...
        try {
            ...
            TestCase.fail("Constructor allows null employee name");
        } catch (RuntimeException e) {
            // do nothing
        }
    }
    ...
    public void testAssignAndRetrieveSalaryComponents() {
        ...
        TestCase.assertTrue("fail to assign Basic component", emp.getSalary("Basic") == 2000);
        TestCase.assertTrue("fail to assign Medical component", emp.getSalary("Medical") == 350);

        // attempt to retrieve a salary component that does not exist
        try {
            emp.getSalary("Transportation");
            TestCase.fail("fail to catch non-existent salary component ");
        } catch (RuntimeException e) {
            // do nothing
        }
    }
    ...
    public void testCalculationOfTotalPay() {
        ...
        TestCase.assertTrue("fail to calculate total pay ", emp.getTotalPay() == 2850);
